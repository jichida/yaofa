...
export const requestUrlGet = (uri,param,callback)=>{
    request.get(uri+querystring.stringify(param),(error, response, body)=> {
        if(error){
            console.log("requestget error:" + JSON.stringify(error));
            callback(false,error.message);
            return;
...
export const requestUrlpost = (uri,param,callback)=>{
    let baseUrl = config.serverurl + uri;
